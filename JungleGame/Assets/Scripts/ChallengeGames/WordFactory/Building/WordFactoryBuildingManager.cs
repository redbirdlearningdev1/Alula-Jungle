...
public class WordFactoryBuildingManager : MonoBehaviour
{
    ...
    public Polaroid polaroid; // main polaroid used in this game
    ...
    private IEnumerator NewRound()
    {
        ...
        if (playTutorial)
        {
            if (tutorialEvent == 1)
            {
                ...
                AssetReference clip = GameIntroDatabase.instance.buildingIntro1;
        ...
        else if (!GameManager.instance.practiceModeON)
        {
            ...
            {
                // play start 1
                AssetReference clip = GameIntroDatabase.instance.buildingStart1;
        ...
        if (playTutorial)
        {
            if (tutorialEvent == 1)
            {
                ...
                AssetReference clip = GameIntroDatabase.instance.buildingIntro2;
        ...
        if (playTutorial)
        {
            if (tutorialEvent == 1)
            {
                ...
                AssetReference clip = GameIntroDatabase.instance.buildingIntro4;
    ...
    private IEnumerator PostRound(bool win)
    {
        ...
        if (win)
        {
            ...
            if (showChallengeWordLetters)
            {
                ...
                for (int i = 0; i < polaroid.challengeWord.elkoninCount; i++)
                {
                    ...
                    AudioManager.instance.PlayFX_oneShot(AudioDatabase.instance.CoinDink, 0.5f, "coin_dink", (1f + 0.25f * i));
                    yield return new WaitForSeconds(1f);
                    currentCoins[i].LerpScale(new Vector2(1f, 1f), 0.25f);
                }
                yield return new WaitForSeconds(0.2f);

                // read word aloud to player
                if (currentPair.word2.audio != null)
                    AudioManager.instance.PlayTalk(currentPair.word2.audio);
                // start wiggle
                polaroid.ToggleWiggle(true);

                CoroutineWithData<float> cd = new CoroutineWithData<float>(AudioManager.instance, AudioManager.instance.GetClipLength(currentPair.word2.audio));
                yield return cd.coroutine;
            ...
            if (playTutorial)
            {
                if (tutorialEvent == 1)
                {
                    ...
                    AssetReference clip = GameIntroDatabase.instance.buildingIntro5;
        ...
        if (playTutorial && tutorialEvent > 1 || !playTutorial)
        {
            ...
            else
            {
                ...
                {
                    List<AssetReference> clips = GameIntroDatabase.instance.buildingReminderClipsChapters1_4;
                ...
                {
                    List<AssetReference> clips = GameIntroDatabase.instance.buildingReminderClipsChapter5;
                    clip = clips[Random.Range(0, clips.Count)];
    ...
    private IEnumerator WinRoutine()
    {
        ...
        if (playTutorial)
        {
            ...
            {
                { "challengegame_name", GameType.WordFactoryBuilding.ToString() },
                { "stars_awarded", 0 },
